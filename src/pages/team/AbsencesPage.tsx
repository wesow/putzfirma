        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-0 overflow-hidden relative group hover:shadow-md transition-all">
            ...
            <div className="p-3 pl-4 flex justify-between items-center">
                <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-md flex items-center justify-center border border-white shadow-sm ${style.iconBg} ${style.iconColor}`}>
                        {style.icon}
                    </div>
                    <div>
                        <h4 className="font-bold text-slate-900 text-[13px] leading-none mb-1.5">
                            {data.employee.firstName} {data.employee.lastName}
                        </h4>
                        <div className="flex items-center gap-2">
                            <span className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase border ${style.badgeClass}`}>
                                {style.label}
                            </span>
                ...
                <button 
                    ...
                    className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all sm:opacity-0 group-hover:opacity-100"
                >
                    <Trash2 size={16}/>
                </button>
            </div>
...
export default function AbsencesPage() {
    ...
    return (
        <div className="page-container pb-safe">
            ...
            <div className="content-grid animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="lg:col-span-8 space-y-4">
                    ...
                    {loading ? (
                        <div className="flex-1 flex flex-col items-center justify-center py-20 bg-white rounded-lg border border-slate-100">
                            <Loader2 className="animate-spin text-blue-600 mb-3" size={32} />
                            <span className="label-caps">Lade Daten...</span>
                        </div>
                    ) : absences.length === 0 ? (
                        ...
                        <form onSubmit={(e) => handleCreate(e, false)} className="p-5 space-y-5">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    <button type="button" onClick={() => setType('VACATION')} className={`flex flex-col items-center gap-1 py-2.5 rounded-lg border transition-all ${type === 'VACATION' ? 'bg-orange-50 border-orange-200 text-orange-600 shadow-sm' : 'bg-white border-slate-200 text-slate-400 hover:border-orange-100'}`}>
                                        <Palmtree size={18}/>
                                        <span className="text-[9px] font-black uppercase">Urlaub</span>
                                    </button>
                                    <button type="button" onClick={() => setType('SICKNESS')} className={`flex flex-col items-center gap-1 py-2.5 rounded-lg border transition-all ${type === 'SICKNESS' ? 'bg-red-50 border-red-200 text-red-600 shadow-sm' : 'bg-white border-slate-200 text-slate-400 hover:border-red-100'}`}>
                                        <Thermometer size={18}/>
                                        <span className="text-[9px] font-black uppercase">Krank</span>
                                    </button>
                                    <button type="button" onClick={() => setType('OTHER')} className={`flex flex-col items-center gap-1 py-2.5 rounded-lg border transition-all ${type === 'OTHER' ? 'bg-slate-100 border-slate-300 text-slate-700 shadow-sm' : 'bg-white border-slate-200 text-slate-400 hover:bg-slate-50'}`}>
                                        <Briefcase size={18}/>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <input required type="date" value={start} onChange={e => setStart(e.target.value)} className="input-standard font-bold text-[12px]"/>
                                </div>
                                <div className="space-y-1.5">
                                    <label className="label-caps !ml-0">Bis (inkl.)</label>
                                    <input required type="date" value={end} onChange={e => setEnd(e.target.value)} className="input-standard font-bold text-[12px]"/>
                                </div>
                            </div>
                            ...
                            <button 
                                type="submit"
                                disabled={submitting}
                                className="btn-primary w-full justify-center py-2.5 shadow-lg shadow-blue-600/20 uppercase tracking-[0.1em] font-black text-[11px]"
                            >
                                {submitting ? <Loader2 className="animate-spin" size={18} /> : <><CheckCircle2 size={18} className="mr-2" /> Planung speichern</>}
                            </button>
                        </form>
                    </div>
                </div>
            ...
            {conflictData && (
                <div className="modal-overlay">
                    <div className="modal-content !max-w-2xl border-t-4 border-t-amber-500">
                        <div className="modal-header !bg-amber-50/30">
                            <div className="flex items-center gap-3">
                                <div className="stat-icon-box bg-amber-500 text-white shadow-md">
                                    <AlertTriangle size={18}/>
                                </div>
                                <div>
                                    <h2 className="text-sm font-black uppercase tracking-tight text-amber-900 leading-none">Konflikte erkannt</h2>
                                    <p className="text-[10px] font-bold text-amber-600 uppercase mt-1">Laufende Jobs während der Abwesenheit</p>
                                </div>
                            </div>
                            <button onClick={() => setConflictData(null)} className="p-1.5 hover:bg-amber-100 rounded-lg text-amber-900 transition-all"><X size={20}/></button>
                        </div>
                        
                        <div className="modal-body !p-5 space-y-4">
                            <div className="p-3.5 bg-amber-50 rounded-lg border border-amber-100 flex gap-3">
                                <Info size={18} className="text-amber-600 shrink-0 mt-0.5" />
                                <p className="text-[12px] text-amber-900 leading-relaxed font-medium">
                                    Der Mitarbeiter ist für <strong>{conflictData.conflicts.length} Einsätze</strong> eingeteilt. Bitte weisen Sie diese neu zu oder lassen Sie sie unbesetzt.
                                </p>
                            </div>

                            <div className="max-h-[300px] overflow-y-auto space-y-2 custom-scrollbar pr-1">
                                {conflictData.conflicts.map((conflict: any) => (
                                    <div key={conflict.id} className="p-3 bg-white border border-slate-200 rounded-lg flex flex-col md:flex-row items-center gap-3 hover:border-amber-300 transition-colors">
                                        <div className="flex-1 min-w-0 text-left">
                                            ...
                                            <div className="font-bold text-slate-900 text-[12px] truncate leading-tight">
                        ...
                        <div className="modal-footer !bg-slate-50 flex justify-between items-center px-5 py-3">
                            <div className="flex flex-col">
                                <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest">Lösung</span>
                                <span className="text-[11px] font-bold text-slate-900">{Object.keys(reassignments).length} / {conflictData.conflicts.length} gelöst</span>
                            </div>
                            <div className="flex gap-2">
                                <button onClick={() => setConflictData(null)} className="btn-secondary !py-2 !px-4 text-[11px]">Abbruch</button>
                                <button onClick={() => handleCreate(undefined, true)} className="btn-primary !bg-amber-600 !border-amber-700 shadow-md !py-2 !px-5 text-[11px]">
                                    <CheckCircle2 size={14} className="mr-1.5"/> Alle speichern
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <ConfirmModal 
                ...
                message="Möchten Sie diese Abwesenheit wirklich entfernen? Der Mitarbeiter steht danach sofort wieder zur Verfügung."
                variant="danger"
